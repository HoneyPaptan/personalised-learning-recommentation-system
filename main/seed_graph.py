...
user_id = cursor.execute("SELECT id FROM users LIMIT 1").fetchone()
if not user_id:
    cursor.execute("INSERT INTO users (name) VALUES ('Learner')")
...
conn.commit()
conn.close()
print("Successfully seeded database with historical mastery data!")
